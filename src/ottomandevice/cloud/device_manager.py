...
def _avatar_health_snapshot() -> dict[str, Any] | None:
    try:
        ...
        manager = AvatarManager.get_instance_optional()
        if manager is None:
            return None
        return manager.health_report()
    except Exception:
...
def _speech_health_snapshot() -> dict[str, Any] | None:
    try:
        from ottomandevice.plugins.speech.manager import SpeechManager

        manager = SpeechManager.get_instance_optional()
        if manager is None:
            return None
        return manager.health_report()
    except Exception:
...
def _ai_health_snapshot() -> dict[str, Any] | None:
    try:
        from ottomandevice.plugins.ai.manager import AIManager

        manager = AIManager.get_instance_optional()
        if manager is None:
            return None
        return manager.health_report()
    except Exception:
