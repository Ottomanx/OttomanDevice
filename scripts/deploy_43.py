...
def deploy_cloud_modules() -> None:
    mapping = {
        "s43_device_manager.py": "src/ottomandevice/cloud/device_manager.py",
        "s43_cloud_init.py": "src/ottomandevice/cloud/__init__.py",
        "s43_tests.py": "tests/unit/test_device_cloud_manager.py",
    }
    for source, destination in mapping.items():
        write(destination, (ROOT / "scripts" / source).read_text(encoding="utf-8"))


def patch_event_bus_cloud_events() -> None:
    path = ROOT / "src/ottomandevice/core/event_bus.py"
    text = path.read_text(encoding="utf-8")
    if "class HeartbeatReceived" in text:
        print("event_bus cloud events already patched")
        return
    marker = "@dataclass(frozen=True)\nclass Subscription:"
    cloud_events = '''
...
'''
    if marker not in text:
        raise RuntimeError("event_bus patch marker not found")
    path.write_text(text.replace(marker, cloud_events + "\n\n" + marker), encoding="utf-8")
    print("patched", path)


def patch_lifecycle_cloud() -> None:
    path = ROOT / "src/ottomandevice/core/lifecycle.py"
    text = path.read_text(encoding="utf-8")
    if "DeviceCloudManager" in text:
        print("lifecycle cloud already patched")
        return
    ...
    old_register = '''        heartbeat = HeartbeatService(self._supabase)
...
'''
    new_register = '''        from ottomandevice.cloud.device_manager import CloudRuntimeHooks, DeviceCloudManager
...
'''
    if old_register not in text:
        raise RuntimeError("lifecycle cloud register marker not found")
    ...
    def _request_runtime_restart(self) -> None:
        """Request a runtime restart via remote command."""
        self._logger.info("Runtime restart requested by cloud command")
        raise SystemExit(0)

'''
    if "_request_runtime_restart" not in text:
        text = text.replace("    def _shutdown(self) -> None:", restart_method + "    def _shutdown(self) -> None:")

    path.write_text(text, encoding="utf-8")
    print("patched", path)


def patch_core_init_cloud() -> None:
    path = ROOT / "src/ottomandevice/core/__init__.py"
    text = path.read_text(encoding="utf-8")
    if "HeartbeatReceived" in text:
        print("core init cloud already patched")
        return
    text = text.replace(
        "    DeviceProfileUpdated,\n)\n",
...
if __name__ == "__main__":
    patch_event_bus_cloud_events()
    deploy_cloud_modules()
    patch_lifecycle_cloud()
    patch_core_init_cloud()
